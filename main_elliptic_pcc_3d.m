...
[~, ~, ~] = mkdir('./Export');
export_path = './Export/Elliptic_PCC_3D';
[~, ~, ~] = mkdir(export_path);

mesh_type = pd.polydim.pde_tools.mesh.pde_mesh_utilities.MeshGenerator_Types_3D.cubic;
method_type = polydim.pde_tools.local_space_pcc_3_d.MethodTypes.vem_pcc;
...
geometry_utilities_config.tolerance2_d = 1.0e-15;
...
max_relative_volumes = [0.1, 0.05, 0.02]; 
errors = [];

for ref=1:length(max_relative_volumes)
    
    %% Create Mesh

    max_relative_volume = py.float(max_relative_volumes(ref));
    [mesh, mesh_data, mesh_geometric_data] = create_mesh(gedim, polydim, geometry_utilities, mesh_utilities, mesh_type, max_relative_volume, import_path, pde_domain);
    ...
    do_fs_data_indices = dof_manager.compute_cells_do_fs_indices(do_fs_data, py.int(3));

    %% Assemble

    num_cell3D = int64(mesh.cell3_d_total_number());
    num_dofs = int64(do_fs_data.number_do_fs);
    num_strongs = int64(do_fs_data.number_strongs);
    cell_dimension = py.int(3);

    global_rhs = zeros(num_dofs, 1);
    solution_dirichlet = zeros(num_strongs, 1);

    global_lhs_values = cell(num_cell3D, 1);
    global_lhs_rows = cell(num_cell3D, 1);
    global_lhs_cols = cell(num_cell3D, 1);

    dirichlet_lhs_values = cell(num_cell3D, 1);
    dirichlet_lhs_rows = cell(num_cell3D, 1);
    dirichlet_lhs_cols = cell(num_cell3D, 1);
    ...
    for c = 1:num_cell3D
        ...
        local_space_data = polydim.pde_tools.local_space_pcc_3_d.create_local_space(geometry_utilities_config.tolerance1_d, geometry_utilities_config.tolerance2_d, ...
            geometry_utilities_config.tolerance3_d, mesh_geometric_data, cell_index, reference_element_data);
        ...
        points = double(cell3_d_internal_quadrature.points);
        ...
        local_a = local_a + k_max * polydim.pde_tools.local_space_pcc_3_d.stabilization_matrix(reference_element_data, local_space_data);
    ...
    cell3_ds_error_l2 = zeros(num_cell3D, 1);
    cell3_ds_norm_l2 = zeros(num_cell3D, 1);
    cell3_ds_error_h1 = zeros(num_cell3D, 1);
    cell3_ds_norm_h1 = zeros(num_cell3D, 1);
    mesh_size = 0.0;
    ...
    for c = 1:num_cell3D
        ...
        local_space_data = polydim.pde_tools.local_space_pcc_3_d.create_local_space(geometry_utilities_config.tolerance1_d, geometry_utilities_config.tolerance2_d, ...
            geometry_utilities_config.tolerance3_d, mesh_geometric_data, cell_index, reference_element_data);
        basis_functions_values = double(polydim.pde_tools.local_space_pcc_3_d.basis_functions_values(reference_element_data, local_space_data, polydim.vem.pcc.ProjectionTypes.pi0k));
        basis_functions_derivative_values = polydim.pde_tools.local_space_pcc_3_d.basis_functions_derivative_values(reference_element_data, local_space_data);
        basis_functions_derivative_values = {double(basis_functions_derivative_values{py.int(0)}), double(basis_functions_derivative_values{py.int(1)}), double(basis_functions_derivative_values{py.int(2)})};
        cell3_d_internal_quadrature = polydim.pde_tools.local_space_pcc_3_d.internal_quadrature(reference_element_data, local_space_data);

        weights = double(cell3_d_internal_quadrature.weights);
        points = double(cell3_d_internal_quadrature.points);
        ...
        cell3_ds_error_l2(c) = weights * local_error_l2;
        cell3_ds_norm_l2(c) = weights * local_norm_l2;
        ...
        local_error_h1 = ((basis_functions_derivative_values{1} * local_solution_do_fs - exact_derivative_solution_values{1}').^2 ...
            ...
            + (basis_functions_derivative_values{3} * local_solution_do_fs - exact_derivative_solution_values{3}').^2);
        ...
        local_norm_h1 = ((basis_functions_derivative_values{1} * local_solution_do_fs).^2 + ...
            ...
            (basis_functions_derivative_values{3} * local_solution_do_fs).^2);

        cell3_ds_error_h1(c) = sum(weights * local_error_h1);
        cell3_ds_norm_h1(c) = sum(weights * local_norm_h1);

        if mesh_geometric_data.cell3_ds_diameters{cell_index} > mesh_size
            mesh_size = mesh_geometric_data.cell3_ds_diameters{cell_index};
        end
    end


    error_l2 = sqrt(sum(cell3_ds_error_l2));
    norm_l2 = sqrt(sum(cell3_ds_norm_l2));
    error_h1 = sqrt(sum(cell3_ds_error_h1));
    norm_h1 = sqrt(sum(cell3_ds_norm_h1));

    errors = [errors; table(num_cell3D, num_dofs, num_strongs, mesh_size, error_l2, error_h1, norm_l2, norm_h1, residual_norm)];
    ...
    vtk_utilities.export_solution_3(strcat([export_path, '/Solution_', int2str(int64(method_type.value)), '_', int2str(int64(method_order)), '_', int2str(ref)]), ...
        ...
        py.numpy.squeeze(py.numpy.array(cell3_ds_error_l2, pyargs('dtype', py.numpy.float64))), ...
        py.numpy.squeeze(py.numpy.array(cell3_ds_error_h1, pyargs('dtype', py.numpy.float64))));
